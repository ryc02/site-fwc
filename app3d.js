...
// Frases emocionais por cor — conectam o produto ao ambiente do cliente
const colorHints = {
    branco: 'Clássico — combina com qualquer ambiente',
    preto: 'Moderno — ideal para salas e quartos contemporâneos',
    titanio: 'Sofisticado — o favorito para ambientes industriais',
    cerejeira: 'Aconchegante — perfeito para salas com madeira',
    imbuia: 'Elegante — para quem busca um toque rústico refinado',
    ouro_velho: 'Premium — o mais vendido para salas clássicas',
    mogno: 'Nobre — destaque para ambientes com personalidade',
    azul: 'Suave — ideal para quartos infantis e lavabos',
    rosa: 'Delicado — perfeito para quartos femininos',
    verde: 'Refrescante — combina com plantas e ambientes naturais',
    palha: 'Neutro — versátil para qualquer decoração',
    marfim: 'Acolhedor — tom quente que ilumina o ambiente'
};
...
document.querySelectorAll('.color-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        // Atualiza a frase emocional da cor
        if (colorHintEl) {
            colorHintEl.textContent = colorHints[currentColor] || '';
        }
...
// Resolve problema de links do TikTok no Desktop (about:blank) via Modal QR Code
...
if (!isMobile) {
    ...
    if (tiktokBtn && qrModal && closeQr && qrImage) {
        tiktokBtn.addEventListener('click', (e) => {
            e.preventDefault(); // Impede de abrir o link que dá erro no desktop
            
            // Pega a URL que seria acessada (já com ou sem parâmetros, no caso o link principal)
            const url = tiktokBtn.href;
            
            // Gera o QR code na hora usando a API
